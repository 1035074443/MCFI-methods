function MVF_blk = MCPM_BMA_3DRS(imC,imR,MVF_blk_pre,blk_sz,enb_sz)
...
imC_pad = cat(3,imextend(imC(:,:,1),ext_sz),imextend(imC(:,:,2),ext_sz),...
    imextend(imC(:,:,3),ext_sz));
imR_pad = cat(3,imextend(imR(:,:,1),ext_sz),imextend(imR(:,:,2),ext_sz),...
    imextend(imR(:,:,3),ext_sz));
